...
from .assessment import assess_openai, review_packet
...
def main() -> None:
    ...
    for criterion in load_criteria(rubric):
        ...
        assessment = assess_openai(packet, args.model) if args.provider == "openai" else None
    ...
    output = {"schema_version": SCHEMA_VERSION, "document_id": phase04["document_id"], "created_at": datetime.now(timezone.utc).isoformat(),
              "assessment_provider": args.provider, "assessment_model": args.model if args.provider != "none" else None,
